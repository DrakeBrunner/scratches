 ...
 * 
 *         <P>
 *         UDatagramSocket provides an unreliable network layer; specifically,
 *         it provides a UDP socket that behaves exactly like a standard UDP
 *         sockets except that it subject the messages to
 *         </P>
 *         <ul>
 *         <li>loss
 *         <li>reordering
 ...
 * 
 *         <P>
 *         UDatagramSocket extends the DatagramSocket class, therefore; you
 *         should be able to use it just like a regular DatagramSocket. The
 *         methods that are overridden are send and close. The send in
 *         UDatagramSocket does not actually send the datagram but adds it to a
 *         queue that is being checked by a different thread called the
 *         NetworkSender. The NetworkSender thread checks the message queue for
 *         any new entries. If there are any then it goes through the steps of
 *         seeing if the datagram will undergo any type of error, and then sends
 *         it, calling the send method in DatagramSocket.
 *         </P>
 */
public class UDatagramSocket extends DatagramSocket {
    ...
    // Probability that a packets will be reordered, only for pipelined
    // simulation
    ...
    /**
     * Default Constructor for the class. It creates a UDP socket, binds it to a
     * remote address
     * 
     * @throws SocketException
     *             : in case a socket exception occurred
     */
    ...
    /**
     * Constructor for the class. It creates a UDP socket, binds it to a remote
     * address
     * 
     * @param localPort
     *            : a local port socket is bound to
     * @throws SocketException
     *             : in case a socket exception occurred
     */
     ...
     *            : a local port socket is bound to
     * @param remoteAddress
     *            : remote address that the socket is bound to
     * @throws SocketException
     *             : in case a socket exception occurred
     */
    ...
    private void completeSetUp() throws SocketException {
        if (DEBUG)
            System.out.println("UnreliableDatagramSocket:  Version " + version);
    ...
    private void readConfigFile() {
        ...
        Scanner file = null;
        try {
            file = new Scanner(new BufferedReader(new FileReader(
                    SOCKET_CONFIG_FILE_NAME)));
        }
        catch (IOException ioe) {

            System.err
                    .println("Config file could not be found! Using Default values.");
            return;
        }

        Pattern comment = Pattern.compile("^#"); // Pattern used to find comment
                                                 // lines
        Pattern pat;
        Matcher mat;
        String currLine;
    ...
    /**
     * Overrides send method in DatagramSocket. Instead of sending the packet,
    ...
    public void send(DatagramPacket packet) {
        ...
        if (DEBUG)
            System.out.println("\"Sending!\" Actually queuing.");
    ...
    /**
     * Calls super class close method. Should only be called when all threads
     * using the super class object have stopped.
     */
    ...
    /**
     * Dequeue an object from queue of datagrams to be sent. Then introduces the
     * message to error(s) if conditions are met.
     * 
        ...
        try {
            ...
            if (modMsg) {
                ...
                if (loseFirstMsg) {
                    ...
                    if (msg.errorsCount >= MAX_ERRORS) { // no errors if msg
                                                         // already
                    ...
                    if ((!outBuf.isEmpty())
                            && (generator.nextDouble() < reorderProb)) {
                    ...
                    if (generator.nextDouble() < duplicateProb) {
                        ...
                        Message duplicateMsg = new Message(new DatagramPacket(
                                msg.packet.getData(), msg.packet.getLength(),
                                msg.packet.getAddress(), msg.packet.getPort()));

                        duplicateMsg.errorsCount = msg.errorsCount + 1;

                        outBuf.insertElementAt(
                                duplicateMsg,
                                ((int) (outBuf.size() * generator.nextDouble())));
                    }
                }
    ...
    private void callSuperSend(DatagramPacket packet) {
        ...
        try {
            ...
            System.err
                    .println("Socket Exception. Most likely socket is being closed!");
    ...
    /**
     * Corrupts the given message. Done by switching a byte in the message.
     * 
     * @param msg
     */
     ...
     * 
     * This thread sends the messages of outBuf, subjecting them to
     * probabilistic errors (loss, reordering, duplication), maximum message
     * lifetime, and transmission delay. Specifically, it does the following: -
     * calls getHeadMsg to get either the head msg of outBuf or null - Sends the
     * msg on UDP socket.
     * 
     * ------------------------------------------------------------------------
     * ------
    ...
    class NetworkSender extends Thread {
        ...
        public void run() {
            ...
            if (DEBUG)
                System.out.println("Run thread for NetworkSender running!");
            ...
            while (!stopSender) {
                ...
                try {
                    ...
                    if (DEBUG)
                        System.err
                                .println("InterruptedException in NetworkedSender");
 ...
 * Class Message
 * 
 * Stores message data and number of errors the message has gone through.
 * 
 * Attributes : ------------ data : Message data errorsCount: Number of errors
