...
（１）関数tokeizer
     １行分の文字列 line[] をトークンと呼ぶ単語に分解する．
（２）関数setTokens
　　　各トークンを分類し，その種別と値を設定する．
ここで，トークンとは，区切り文字（空白，タブ，カンマ）で
区切られた一連の文字列である．
たとえば，文字列"load  FNAME1,0x10\n"は，"load","FNAME3","0x10"の
３つのトークンに分解できる．
トークンの種別は，整数（１０進数または１６進数（数字の列の先頭に#を付与)），
keyword.hであらかじめ定義されたキーワード，ラベル(大文字英字ではじまり，
大文字英字または数字８文字以内のキーワード以外の文字列），
未定義（その他の文字列）の４種類とする．
また，１０進数と１６進数の場合は，符号なし整数値，キーワードの場合は
キーワードの識別子，ラベルの場合は文字数をトークンの値として設定する．
上の例では，"load"は種別はキーワードで値が３，"FNAME3"は種別はラベルで値は６，
"0x100"は種別は整数で値は１６である．

トークンの処理に関する定数，データ型はtoken.hの定義を用いる．
また，キーワードの定義表は，関数setTokenの引数(KeyWD k[])を用いる．
...
/* 読み込んだ文字列lineからトークンを１つ読み出して，
 * 文字列strに最大TOKENSIZE文字数まで書き込む．
   ただし, 文字数が0でなく' '（空白）または','（カンマ）
   または'\t'（タブ）を読み込んだらbreak;
*/
...
int isLabel(char *s) {
    int n = 0;
    if ((*s >= 'A') && (*s <= 'Z')) {   /* Begin with upper case? */
        s++; n++;
        while (*s != '\0') {

            // When n < 8 AND *s is A-Z or 0-9
            if ( (n < 8) && (((*s >= 'A') && (*s <= 'Z')) ||
                             ((*s >= '0') && (*s <= '9'))) ) {
                s++; n++;
            }
    ...
    // If it doesn't begin with upper case in the first place
    else
        return 0;
}
...
// (* ここに解答を書き加える *)
        // When it's not a keyword, check if it's Lable
        else {
            ...
            // Neither Keyword nor Lable
            else {
                x = dectoi(token[i].data);
                // If string is a decimal
                if (x >= 0) {
                    token[i].type = TKNINT;
                    token[i].value = x;
                }
                // There's something other than numbers in string.
                // It's possible that it's a hexadecimal number
                else {
                    ...
                    // If nothing applies, it's an undef
                }
            }
        }
// END
