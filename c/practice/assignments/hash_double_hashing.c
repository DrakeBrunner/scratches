...
int HashAdd(Hash *hash, char *key, char *value) {
    ...
    int j = HashCode2(hash, key);
    int count;

    for (count = 0; count < hash->size; count++) {
...
int HashCode2(Hash *hash, char *key) {
    return 8 - (key[0] % 8);
}
    ...
    do {
        ...
        switch (cmd) {
            ...
            case 'h':
                printf("Enter name: ");
                scanf("%s", key);
                h1 = HashCode(hash, key);
                printf("1st Hash Value for %s is %d\n", key, h1);
                h2 = HashCode2(hash, key);
                printf("2nd Hash Value for %s is %d\n", key, h2);
                break;
            case 'q':
                puts("Quitting");
                break;
